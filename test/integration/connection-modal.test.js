...
// The tests below require DigitalCoDeBlock Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying DigitalCoDeBlock Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('DigitalCoDeBlock Link'); // DigitalCoDeBlock Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying DigitalCoDeBlock Link is unavailable (BT)', async () => {
        await loadUri(uri);
